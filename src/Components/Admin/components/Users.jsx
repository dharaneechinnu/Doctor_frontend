...
export default function Users() {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get('https://doctor-backend-eubv.onrender.com/appointments', {
        headers: {
  ...
  const handleDateChange = async (params, newDate) => {
    try {
      ...
      const response = await axios.post(
        'https://doctor-backend-eubv.onrender.com/date',
        {
          _id: params._id,
          date: newDate,
        },
  ...
  const handleSave = async (appointment) => {
    try {
      ...
      const response = await axios.patch(
        'https://doctor-backend-eubv.onrender.com/appointments',
        {
          _id: appointment._id,
          status: 'checked',
  ...
  const columns = [
    { field: '_id', headerName: 'ID', width: 200 },
    {
      ...
      width: 200,
      renderCell: (params) => {
        const userName = params.row.user ? params.row.user.username : 'N/A';
        return userName;
      },
    },
